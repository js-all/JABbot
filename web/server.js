...
io.on('connection', (socket) => {
    ...
    socket.on('GlobalRankReq', id => {
        dbm.getAcc(acc => {
            const currentAcc = acc["<@" + id + ">"];
            if (currentAcc !== undefined) {
                let n = Object.keys(acc).length;
                for (let i in acc) {
                    const e = acc[i];
                    if (i != "<@" + id + ">") {
                        if (currentAcc.msg > e.msg) {
                            n--;
                        } else if (currentAcc.msg == e.msg) {
                            if (currentAcc.points == e.points) {
                                if (currentAcc.money == e.money || currentAcc.money > e.money) {
                                    n--;
                                }
                            } else if (currentAcc.points > e.points) {
                                n--
                            }
                        }
                    }
                }
                socket.emit('GlobalRankRes', {rank: n, on: Object.keys(acc).length});
            }
        });
