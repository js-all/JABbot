            ...
            if (acc["<@"+message.author.id+">"] != undefined) {
                ...
                if (args[0] == '-i') {
                    ...
                    const embed = new Discord.RichEmbed()
                        ...
                        .setTimestamp();
                    var rol = '';
                    for (var k = 0; k < acc["<@"+message.author.id+">"].roles.length; k++) {
                        rol += ' ' + (k + 1) + ' : ' + acc["<@"+message.author.id+">"].roles[k] + ' ,\n';
                    }
                    ...
                    // on equipe un role
                    if (acc["<@"+message.author.id+">"].roles.length >= args[2]) {
                        if (message.guild.roles.find('name', acc["<@"+message.author.id+">"].roles[args[1] - 1]) != undefined) {
                            message.member.addRole(message.guild.roles.find('name', acc["<@"+message.author.id+">"].roles[args[1] - 1]));
                        }
                    }
                } else if (args[0] == '-u') {
                    // on enleve un role
                    if (message.guild.roles.find('name', acc["<@"+message.author.id+">"].roles[args[1] - 1]) != undefined) {
                        message.member.removeRole(message.guild.roles.find('name', acc["<@"+message.author.id+">"].roles[args[1] - 1]));
                    }
                } else {
