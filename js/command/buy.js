                    ...
                    if (args[0] == 'r') {
                        if (args[1] == 'c') {
                            if (args[2] == 'r') {
                                ...
                                if (acc[message.author].money >= 1000) {
                                    // on ajoute le role a la liste des role de l'utilisateure
                                    acc[message.author].roles.push('rouge');
                                    // on enlève l'argent
                                    acc[message.author].money -= 1000;
                                ...
                                // de meme
                                if (acc[message.author].money >= 1000) {
                                    acc[message.author].roles.push('vert');
                                    acc[message.author].money -= 1000;
                                    message.channel.send('vous venez d\'acheter le role  pour 1000m !,' + message.author);
                                } else {
                                    message.reply('vous n\'avez pas assé d\'argent pour acheter cela');
                                ...
                                // le meme chose
                                if (acc[message.author].money >= 1000) {
                                    acc[message.author].roles.push('bleu');
                                    acc[message.author].money -= 1000;
                                    message.channel.send('vous venez d\'acheter le role  pour 1000m !,' + message.author);
                                } else {
                                    message.reply('**vous** n\'avez pas assé d\'argent pour acheter cela');
                                ...
                                if (acc[message.author].money >= 10000) {
                                    // on test si le 4emme parametre est bien une couleure hexadecimal 
                                    if (args[3] != undefined && /#[0-9A-Fa-f]{6}/.test(args[3]) == true) {
                                        acc[message.author].roles.push('--' + message.author.username + '\'s custom ' + args[3] + ' role--');
                                        acc[message.author].money -= 10000;
                        ...
                        if (args[1] == 't') {
                            ...
                            if (acc[message.author].money >= 2500) {
                                if (args[2] != undefined && args[3] == undefined && args[2].indexOf('|') == -1) {
                                    acc[message.author].user = args[2].replace(/-/gi,' ');
                                    acc[message.author].money -= 2500;
