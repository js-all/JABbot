    ...
    _ : (com,acc,prefix,args,message,bot) => {
        if (com === c) {
            if (acc[message.author] != undefined) {
                if (args[0] == '-t') {
                    message.reply('vous avez envoyer ' + acc[message.author].msg + ' msg aux total de puis la creation de votre compte')
                }
                else if (args[0]== '-p') {
                    message.reply('il vous reste ' + (((acc[message.author].lvl * acc[message.author].lvl) + 1) - acc[message.author].msgS) + 'a envoyer pour passé au prochain niveaux');
                }
                else {
